...
static bool TurnBody_InputAccess() {
	auto left = StickState();
	auto right = StickState();
...
static bool TurnBody_AnimationAccess(zCModel* model) {
	...
	if (!isBackward) {
		auto wm = oCAniCtrl_Human::GetWeaponHitString(player->GetWeaponMode());
		isBackward =
...
void zCVob::DoFrameActivity_Union() {
	...
	if (CanTurnThisModel) {
		bool strafe = xinputDevice.isStrafeButtonPressed();
